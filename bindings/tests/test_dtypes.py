...
def test_ciou_distance_bad_inputs():
    with pytest.raises(TypeError, match=_BOXES_NOT_NP_ARRAY):
        ciou_distance("foo", "bar")
    with pytest.raises(Exception):
        try:
            ciou_distance(np.random.random((100, 23)), np.random.random((100, 23)))
        except:  # noqa: E722
            raise RuntimeError()


def test_ciou_distance_dtype():
    boxes1 = np.random.random((100, 4))
    boxes2 = np.random.random((100, 4))
    with pytest.raises(TypeError):
        ciou_distance(
